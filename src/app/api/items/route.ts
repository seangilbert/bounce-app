...
/**
 * Public catalog for the first-party storefront: the active operator's active
 * items. Availability is annotated when a range is given:
 *   `?start=YYYY-MM-DD&end=YYYY-MM-DD`  (multi-day), or
 *   `?date=YYYY-MM-DD`                  (single-day shorthand).
 *
 * Shares its logic with the public API (`/api/v1/catalog`) via
 * `buildStorefrontCatalog` — one source of truth.
 */
