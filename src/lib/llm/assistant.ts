...
function catalogForPrompt(items: ItemAvailability[], date: string): string {
  return items
    .map(
      (i) =>
        `- [${i.id}] ${i.name}` +
        ...
        `${i.availability.available} available on ${date}` +
...
function buildUserPrompt(inquiry: Inquiry, catalog: string): string {
  const parts = [`Customer inquiry: "${inquiry.message}"`, ``, `Event date: ${inquiry.eventDate}`];
  ...
  parts.push(``, `Catalog (id, name, price, availability on the event date):`, catalog);
  return parts.join("\n");
}
...
export async function handleInquiry(inquiry: Inquiry): Promise<InquiryResult> {
  ...
  const catalog = await availabilityForOperator(operator.id, inquiry.eventDate);
  ...
  const response = await client.messages.parse({
    ...
    messages: [{ role: "user", content: buildUserPrompt(inquiry, catalogForPrompt(catalog, inquiry.eventDate)) }],
  });
  ...
  // Recompute the quote from authoritative catalog prices — never trust the
  // model's numbers. Keep only lines that map to a real, active catalog item.
  const lines: QuoteLine[] = [];
  for (const li of out.lineItems) {
    const item = li.matched ? catalogById.get(li.itemId) : undefined;
    if (!item) continue;
    const unitPrice = item.basePrice;
    lines.push({ itemId: item.id, name: item.name, quantity: li.quantity, unitPrice, lineTotal: unitPrice * li.quantity });
  }
  ...
  if (hoursUntil(inquiry.eventDate) < MIN_LEAD_HOURS) reasons.push("event is within 48 hours");
