...
- [ ] **Photo lightbox / carousel** — clicking a product **thumbnail** (storefront item cards, the chat quote card, and the checkout "Review & book" summary — optionally the operator inventory list) opens a **lightbox** showing a **carousel of all of that item's photos** (`items.images`), not just the primary. Build a shared `Lightbox` component: prev/next arrows, swipe on touch, keyboard nav (←/→, Esc), an image counter/dots, and scrim click-to-close; open on click of any thumbnail with ≥1 image. Keep it accessible (focus trap, `aria`) and lazy-load off-screen slides. Builds directly on the inventory-photos work.
