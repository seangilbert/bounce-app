...
- [x] **Wire the Inquiries inbox actions** — "Send reply" records the operator's (edited) reply + marks the inquiry `replied` (clears it from "needs you"); "Dismiss" → `dismissed` (hidden). Operator-scoped server actions; dashboard/badge counts update. _Follow-up: actual customer delivery via email (Resend) — until then it's inbox management + a recorded reply._
