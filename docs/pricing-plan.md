...
**Phase 2 as built (2026-08-17 — typecheck + lint clean, 436 tests green):**

- **SMS gate (`smsChannel`, Solo+)** — shipped dormant ahead of Twilio, the same pattern as the contract gate. Three enforcement points: the composer's Text toggle is hidden off-plan (`inquiries/page.tsx`), the bootstrap action refuses to start an SMS thread, and the **inbound webhook suppresses the billable outbound reply** on the downgrade edge (thread opened while paid, replied to after lapse) — the message still lands in the thread with escalation/alerts intact, so nothing is silently lost. The operator reply action falls back to email when the customer left one, and refuses *before* persisting when they didn't (no recorded replies the customer never got). Covered by a webhook test for the suppression path + capability tests.
- **Annual billing (2 months free)** — `yearlyPriceCents`/`stripeYearlyLookupKey` on paid plans ($390 / $790, `solo_yearly`/`growing_yearly`); `billing/checkout` accepts `{interval}`; a Monthly / Annual toggle on the pricing cards carries `?interval=year` through signup → checkout, and signup's fine print shows the annual price with a switch link. `setup_billing.mjs` extended to create/sync yearly prices on the same products. ⚠️ **Owner action: re-run the script once more** (test now, live at go-live) — the yearly prices don't exist in Stripe until it runs; until then an annual checkout 503s cleanly.
- **Item-cap meter** — the inventory header now reads "N of 5 items on the Free plan" for capped plans, and from one-item-short onward admins see a runway banner ("You're one item away…" / "You've used all 5…") with an Upgrade CTA — the cap announces itself before the create action ever rejects.

**Cost-side levers also shipped (2026-08-17):** `ASSISTANT_MODEL` split into three per-task dials in `lib/llm/assistant.ts` — the quote agent stays on the strongest tier (`QUOTE_MODEL`, it's the demo and carries the judgment calls), the operator-reviewed copilot draft is independently tunable (`DRAFT_MODEL`), and **reminder intros moved to Haiku 4.5** (`REMINDER_MODEL`, ~1/5 the price for a two-sentence copywriting task). **Prompt caching** added on the quote agent's grounded system prompt (the bulk of every turn's input; turns 2+ of a conversation read it at ~10% of the input rate), with the copilot draft structured as base-prefix + mode-switch blocks so it shares the same cache entry. Net: the platform's largest AI cost (multi-turn quote conversations) drops materially with zero quality change, before any model downgrade is ever needed.

Remaining: Phase 3 (trial-for-all, direct-charges migration, price re-check with real conversion data) — unchanged below.
